...
accounts, cloud synchronization, or automatic crash reporting.
...
- PDF files that you open, drag into the application, select through an
  operating-system file dialog, or open through a PDF file association.
- Images that you choose to insert into a document.
- Passwords that you enter to open encrypted PDF files.
- File names, local file paths, page content, annotations, bookmarks, form
  values, metadata, and other information contained in the files you open.
...
Passwords for encrypted PDFs are kept in memory only for the time needed to
open the document. They are not saved or transmitted.
...
- Crash-recovery snapshots containing document bytes, annotations, file names,
  and local source paths. The application keeps up to five local recovery
  revisions for a document window.
- PDFs, split documents, exports, and optional backup copies that you choose to
  save.

Recovery data and preferences remain on your device. Recovery snapshots are
normally removed after work is successfully saved or explicitly discarded.
Stale snapshots may remain available for manual recovery or deletion.
...
- View, restore, or delete recovery snapshots.
...
Because the publisher does not collect or maintain a server-side copy of your
information, there is no remote user profile or cloud-held personal data for
the publisher to access, correct, export, or delete.
...
backups, and the security of the folders where you save documents.
